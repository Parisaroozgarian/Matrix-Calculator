...
def parse_matrix(matrix_str):
    ...
    try:
        ...
        raise ValueError(
            f"Invalid matrix format: {str(e)}. Ensure you use a 2D list, e.g., [[1, 2], [3, 4]]."
        )
...
class MyList:
    ...
    def __matmul__(self, m2):
        ...
        if self.sizecol != m2.sizerow:
            raise ValueError(
                f"Matrix dimensions incompatible for multiplication: ({self.sizerow}x{self.sizecol}) @ ({m2.sizerow}x{m2.sizecol})"
            )

        result = MyList(self.sizerow, m2.sizecol)
        for i in range(self.sizerow):
            for j in range(m2.sizecol):
                total = sum(self.__list[i][k] * m2.__list[k][j]
...
def index():
    ...
    if request.method == 'POST':
        try:
            ...
            if operation == "add":
                ...
                if not matrix1.is_square():
                    raise ValueError(
                        "Eigenvectors can only be calculated for square matrices"
                    )
    ...
    return render_template('index.html', result=result_str, errors=errors)
